...
// Check for a new version of the OpenPhorum software. If there's a new version,
...
print '<div class="OpenPhorumAdminTitle">Check for new OpenPhorum version</div>';
print '<br/>';
...
if ($running_type == 'snapshot') {
    print "<br/>If this OpenPhorum installation is run on a production server, " .
          "the OpenPhorum team recommends upgrading to either a stable " .
...
if ($running_type == 'stable' && $new_s) {
    print "<br/>A new stable release is available. The OpenPhorum team " .
...
if ($running_type == 'development' && $new_s && $new_d) {
    print "<br/>There's both a new stable and a new development release " .
          "available. If this OpenPhorum installation " .
          "is run on a production server, the OpenPhorum team recommends " .
...
if ($running_type == 'development' && $new_d && ! $new_s) {
    print "<br/>A new development release is available. If this OpenPhorum " .
          "installation is run on a production server, the OpenPhorum team " .
...
if (! $found_upgrade) {
    print "<br/><h3 class=\"input-form-th\">" .
          "Your OpenPhorum installation is up to date</h3>";
}
?>
