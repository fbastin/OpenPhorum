...
function phorum_mod_google_calendar_create_new_calendar ($data) {
    ...
    $calendar_entry = phorum_mod_google_calendar_render_entry_create_calendar($data);
    $google_url = "http://www.google.com/calendar/feeds/default/owncalendars/full";
    
    //send the entry to google
    $google_response = phorum_mod_google_calendar_gdata_create_calendar($calendar_entry, $google_url);
    ...
    preg_match("/(?:<id>)([^<]+)/i",$google_response,$matches);
        ...
        if (function_exists('event_logging_writelog')) {
            $log_message = "Error creating the Google calendar for the ".$PHORUM["phorum_mod_google_calendar"]["calendars"][$calendar_type][$data["calendar_id"]]["name"];
    ...
    if (function_exists('event_logging_writelog')) {
        $log_message = "Google calendar created for the ".$PHORUM["phorum_mod_google_calendar"]["calendars"][$calendar_type][$data["calendar_id"]]["name"];
        if ($calendar_type == "categories") {
            $log_message .= " category";
        } elseif ($calendar_type == "forums") {
            $log_message .= " forum";
...
function phorum_mod_google_calendar_gdata_create_calendar($calendar_entry, $google_url) {
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, $calendar_entry);
...
function phorum_mod_google_calendar_render_entry_create_calendar ($calendar_data) {
    $calendar_entry = "<entry xmlns='http://www.w3.org/2005/Atom'\n".
        "xmlns:gd='http://schemas.google.com/g/2005'\n".
        "xmlns:gCal='http://schemas.google.com/gCal/2005'>\n".
        "<title type='text'>{$calendar_data["name"]}</title>\n".
        "<gCal:hidden value='false'></gCal:hidden>\n".
        "<gCal:selected value='true' />\n".
        "<gCal:color value='{$calendar_data["color"]}'></gCal:color>\n".
        "</entry>";
