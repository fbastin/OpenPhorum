...
$start_min_ts = $curr_time - (date("G", $curr_time) * 3600) - (date("i",$curr_time) * 60) - (date("s",$curr_time));
...
$week_start = date("w",$start_min_ts);
...
//check each calendar for the next seven days of events
...
for ($day = 0; $day <= 6; $day++) {
    ...
    // if the offset day is outside of the seven day range, bring it back in.
    if ($offset_day > 6) $offset_day = $offset_day - 7;
...
$PHORUM["DATA"]["LANG"]["phorum_mod_google_calendar"]["EventListing"] = $PHORUM["DATA"]["LANG"]["phorum_mod_google_calendar"]["NextSevenEventListing"];
