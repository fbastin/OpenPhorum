{IF phorum_mod_google_calendar->calendars_to_show}
    ...
    .google_calendar_bubble {
        ...
        background-color: {bodybackground};
        padding: 5px;
        border: 4px solid {headerbackcolor};
        }
    </style>
      ...
      <tr>
        <td class="side-column">
            <div class="empty-position">
                {INCLUDE "left_tabs_upper_header"}{INCLUDE "left_tabs_upper_first_row"}
            </div>
        </td>
        <th class="forum-subject">
        ...
        <td class="side-column">
            <div class="empty-position">
                {INCLUDE "right_tabs_upper_header"}{INCLUDE "right_tabs_upper_first_row"}
            </div>
        </td>
      </tr>
      <tr>
        <td class="bottom-side-column"><div class="empty-thin-position">{INCLUDE "left_tabs_lower_bottom_row"}</div></td>
        <td class="bottom-row"><img src="./images/trans.gif" /></td>
        <td class="bottom-side-column"><div class="empty-thin-position">{INCLUDE "right_tabs_lower_bottom_row"}</div></td>
      </tr>
      </table>
      </div>
    ...
    <table cellspacing="0" class="sr-table">
        <tr>
            ...
            <th class="forum-subject" colspan="7">
        ...
        <tr>
            <td class="side-column">
                <div class="empty-position" style="top: 3px;">
            ...
            {LOOP phorum_mod_google_calendar->event_listing_header}
                <td class="google_calendar_td_header" style="font-weight: bold; text-align: center;{IF ALT_EVENT_TD} background-color:{altbackcolor};{/IF}">
        ...
        {LOOP phorum_mod_google_calendar->row_count}
            ...
            {LOOP phorum_mod_google_calendar->event_listings}
                {IF phorum_mod_google_calendar->curr_day 0}
                    <tr>
                    <td class="side-column">
                        <div class="empty-position"  >
                            {INCLUDE "left_tabs_upper_posting_buttons_row"}{INCLUDE "left_tabs_lower_posting_buttons_row"}
                        </div>
                    </td>
                {/IF}
                <?php
                ...
                {IF NOT phorum_mod_google_calendar->curr_row_data->full_day_event}
                    <td valign="top"
                        ...
                        style="border-top: 4px solid {headerbackcolor}; 
                        {! --- If we are on an alternating day, show the alternate background color --- }
                        {IF ALT_EVENT_TD}background-color: {altbackcolor};{/IF}
                        "
                ...
                {/IF}
                {IF phorum_mod_google_calendar->curr_day 6}
                    <td class="side-column" style="top: 3px;">
                        <div class="empty-position">
                            {INCLUDE "right_tabs_upper_posting_buttons_row"}{INCLUDE "right_tabs_lower_posting_buttons_row"}
                        </div>
                    </td>
                    </tr>
                {/IF}
        ...
        <tr>
            <td class="bottom-side-column"><div class="empty-thin-position">{INCLUDE "left_tabs_lower_bottom_row"}</div></td>
            <td colspan="7" class="bottom-row"><img src="./images/trans.gif" /></td>
            <td class="bottom-side-column"><div class="empty-thin-position">{INCLUDE "right_tabs_lower_bottom_row"}</div></td>
        </tr>
    </table>
    </div>
