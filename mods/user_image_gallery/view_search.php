...
// preconditions: $search will be set to text to search for
//                $page will be set to page # to view

// uses the same template as user gallery

// first, grab the list of all keywords

$keywords_array = mod_user_image_gallery_get_keywords_array();

// will be in form $key=>$val, where $key is image number

// now get keys of items that match search criteria

$match_keys = array();

// modified from routine written by "n-regen":
...
// if none found, skip everything and report fail
// TODO

// Retrieve list of all images
$all_images = phorum_api_file_list('image_g', NULL, NULL);
...
// The search function can't tell the difference between approved and unapproved messages. This step takes care of that.

foreach ($match_keys as $id):
...
if ( $end >= $total_items ):
  $end = $total_items - 1;
    ...
    $search_parameter = urlencode($search);
    $url_template = phorum_get_url(PHORUM_ADDON_URL, 'module=user_image_gallery', 'view=search', 'search='.$search_parameter, 'page=%page_num%');
...
for ($i = $start; $i <= $end; ++$i):
    $id = $display_list[$i];
    $images[$id] = $all_images[$id];         // set $images[$id]['file_id'], ['filename'], ['filesize'], and ['add_datetime'] all at once
    $dimensions = NULL;
    ...
    $images[$id]["link"] = phorum_get_url(PHORUM_ADDON_URL, 'module=user_image_gallery', 'view=image', 'image='.$id, 'return_url='.$return_url_encoded, 'from=2' );
    if ( $info[$id]['title'] != '' ):
...
// print recursive_print ("\$images", $images); die(); //debugg
/*
expecting (at least):
FILES->url
FILES->adjustment
...
$PHORUM['DATA']['HEADING'] = $lang['SearchResults'];
